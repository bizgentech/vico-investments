  ...
  return (
    <section id="inventory" className="py-24 bg-[#0A0A0A]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Filters */}
        <div className="flex flex-wrap gap-3 mb-10 p-4 bg-[#141414] border border-[#222]">
          <div className="flex-1 min-w-48 flex items-center gap-2 bg-[#1E1E1E] px-3 py-2 border border-[#333]">
          ...
          <select
            ...
            className="bg-[#1E1E1E] text-white border border-[#333] px-3 py-2 font-condensed text-sm tracking-wide outline-none"
          >
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-[#141414] h-80 animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map(car => {
              ...
              return (
                <div key={car.id} className="car-card bg-[#141414] border border-[#222] overflow-hidden group cursor-pointer">
                  {/* Image */}
                  <div className="relative h-52 overflow-hidden bg-[#1E1E1E]">
                    {images[0] ? (
                      <img src={images[0]} alt={car.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                    ) : (
                      <div className="flex items-center justify-center h-full text-gray-700 font-condensed text-sm">No Image</div>
                    )}
                    {car.featured && (
                      <div className="absolute top-3 left-3 bg-gold text-black font-condensed font-semibold text-xs tracking-widest uppercase px-2 py-1">
                        Featured
                      </div>
                    )}
                  ...
                  <div className="p-5">
                    ...
                    <a href="#appointment"
                       className="block w-full text-center border border-[var(--gold)] text-[var(--gold)] font-condensed tracking-widest uppercase text-sm py-2.5 hover:bg-gold hover:text-black transition-all duration-300">
